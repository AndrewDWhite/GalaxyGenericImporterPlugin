...
class GenericEmulatorPlugin(Plugin):
    ...
    async def update_local_games(self):
        ...
        new_local_games_list = await loop.run_in_executor(None, ListGames().list_all_recursively, self.MyPlatform)
...
def main():
    for my_current_platform_to_setup in Platform:
        my_custom_plugin = GenericEmulatorPlugin
        my_custom_plugin.MyPlatform = my_current_platform_to_setup
        port = 0
        with socket() as s:
            s.bind(('',0))
