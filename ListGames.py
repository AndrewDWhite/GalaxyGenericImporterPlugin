...
class ListGames():
    ...
    def list_all_recursively(self, my_platform):
        logging.info("listing")
        self.mylist=[]
        for emulated_system in self.loaded_systems_configuration:
            if (emulated_system["name"] == my_platform.value) or (not (emulated_system["name"] in Platform._value2member_map_) and (my_platform == Platform.Test)):
